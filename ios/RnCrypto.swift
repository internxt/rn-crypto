...
class RnCrypto: NSObject {
  ...
  private let utils = RnCryptoUtils()
  private let HMAC = RnCryptoHMAC()
  private let keyDerivation = RnCryptoKeyDerivation()
  ...
  @objc func encryptFileToChunks(
    ...
    let operation = EncryptFileToChunksOperation(
      plainFilePath: URL(fileURLWithPath: plainFilePath),
      encryptedFilePaths: destinationPaths.map { URL(fileURLWithPath: $0) },
      hexKey: hexKey,
      hexIv: hexIv,
